...
public:
    ...
    {
        if (!MOPS::math::isfinite(current_position.x()) ||
            !MOPS::math::isfinite(current_position.y()) ||
            !MOPS::math::isfinite(current_position.z())) {
            return false;
        }

        const auto current_cell_vertices_number = numberVertexOnCell[cell_id];
        if (current_cell_vertices_number == 0) {
            return false;
        }

        for (size_t k = 0; k < current_cell_vertices_number; ++k) {
            const auto a_idx = verticesOnCell[cell_id * max_edge + k] - 1;
            const auto b_idx = verticesOnCell[cell_id * max_edge + ((k + 1) % current_cell_vertices_number)] - 1;

            const auto a = vertexCoord[a_idx];
            const auto b = vertexCoord[b_idx];
            const auto surface_normal = MOPS_CROSS(a, b);
            const auto direction = MOPS_DOT(surface_normal, current_position);
            if (direction < 0.0) {
                return false;
            }
        }
    ...
    static inline void GetCellVerticesIdx(
        int cell_id,
        ...
        const size_t* verticesOnCell)
    {
        ...
        const size_t nan = std::numeric_limits<size_t>::max();
        for (int k = current_cell_vertices_number; k < VLA; ++k) {
            current_cell_vertices_idx[k] = nan;
        }
    }

    static inline void GetCellNeighborsIdx(
        int cell_id,
        int current_cell_vertices_number,
        ...
        const size_t* cells_on_cell)
    {
        ...
        for (int k = 0; k < copyN; ++k) {
            const int nid1 = static_cast<int>(cells_on_cell[cell_id * max_edge + k]);
    ...
    static inline bool GetCellVertexPos(
        ...
        const vec3* vertexCoord)
    {
        ...
        for (int v_idx = current_cell_vertices_number; v_idx < VLA; ++v_idx) {
    ...
    static inline vec3 CalcVelocity(
        const size_t* current_cell_vertices_idx,
        ...
        const vec3* cellVertexVelocity)
    {
        ...
        for (int v_idx = 0; v_idx < current_cell_vertices_number; ++v_idx) {
            const auto vid = current_cell_vertices_idx[v_idx];
            const auto vel = cellVertexVelocity[vid * total_ztop_layer + layer];
    ...
    static inline double CalcAttribute(
        const size_t* current_cell_vertices_idx,
        const double* current_cell_vertex_weight,
        const int VLA,
        int current_cell_vertices_number,
        ...
        const double* cellAttribute)
    {
        ...
        for (int v_idx = 0; v_idx < current_cell_vertices_number; ++v_idx) {
            const auto vid = current_cell_vertices_idx[v_idx];
            const auto value = cellAttribute[vid * total_ztop_layer + layer];
            current_point_attr_value += current_cell_vertex_weight[v_idx] * value;
        }
        return current_point_attr_value;
    }

    static inline vec3 CalcRotationAxis(const vec3& position, const vec3& velocity)
    {
        vec3 axis;
    ...
    static inline vec3 CalcPositionAfterRotation(const vec3& position, const vec3& axis, double theta_rad)
    {
