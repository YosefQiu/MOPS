...
namespace MOPS
{
    ...
    std::vector<ImageBuffer<double>> MOPSApp::runRemapping(VisualizationSettings* config)
    {
		...
		auto str = std::to_string(mpasoField->mSol_Front->mCurrentTime);
		str = str + "_" + std::to_string(config->tile_index);
		std::vector<std::string> names = {
				"E: Zonal Velocity", "N: Meridional Velocity", "Velocity Magnitude",
				"Temperature", "Salinity", "None"
			};
		VTKFileManager::SaveVTI(img_vec, config, names, str);
    ...
    std::vector<TrajectoryLine> MOPSApp::runStreamLine(TrajectorySettings* config, std::vector<CartesianCoord>& sample_points)
    {
		...
		std:: cout << " ==== [\u2713] done..." << std::endl;
		// sample_points.clear(); sample_points = lastPtn; cell_id_vec.clear();
		// VTKFileManager::SaveTrajectoryLinesAsVTP(lines, config->fileName);
		// std::cout << " [\u2713]finished running stream line" << std::endl;
		return lines;
	}
