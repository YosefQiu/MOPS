...
void CopyBackAndFree(std::vector<T>& dst, T* dev_ptr, const char* what)
{
    ...
    if (!dst.empty()) {
        CheckCuda(cudaMemcpy(dst.data(), dev_ptr, dst.size() * sizeof(T), cudaMemcpyDeviceToHost), what);
    }
    ...
    const std::vector<size_t>& grid_info)
{
    ...
    if (cell_center_ztop.size() < total_cells * total_layer) {
        Error("[CUDABackend::Kernel]::LaunchCalcCellVertexZtop input size mismatch");
    }

    cell_vertex_ztop.assign(vertex_size * total_layer, 0.0);

    vec3* d_vertex_coord = DeviceAllocAndCopy(grid->vertexCoord_vec, "cudaMalloc/cudaMemcpy vertexCoord");
    vec3* d_cell_coord = DeviceAllocAndCopy(grid->cellCoord_vec, "cudaMalloc/cudaMemcpy cellCoord");
    size_t* d_cells_on_vertex = DeviceAllocAndCopy(grid->cellsOnVertex_vec, "cudaMalloc/cudaMemcpy cellsOnVertex");
    double* d_cell_center_ztop = DeviceAllocAndCopy(cell_center_ztop, "cudaMalloc/cudaMemcpy cellCenterZTop");
    double* d_cell_vertex_ztop = nullptr;

    CheckCuda(cudaMalloc(reinterpret_cast<void**>(&d_cell_vertex_ztop), cell_vertex_ztop.size() * sizeof(double)), "cudaMalloc cellVertexZTop");
    ...
    CheckCuda(cudaGetLastError(), "KernelCalcCellVertexZtop launch");
    CheckCuda(cudaDeviceSynchronize(), "KernelCalcCellVertexZtop sync");

    CopyBackAndFree(cell_vertex_ztop, d_cell_vertex_ztop, "cudaMemcpy/cudaFree cellVertexZTop");
    FreeDev(d_cell_center_ztop, "cudaFree cellCenterZTop");
    FreeDev(d_cells_on_vertex, "cudaFree cellsOnVertex");
    ...
    const std::vector<size_t>& grid_info)
{
    ...
    if (cell_center_attr.size() < total_cells * total_layer) {
        Error("[CUDABackend::Kernel]::LaunchCalcCellCenterToVertex input size mismatch");
    }
    ...
    vec3* d_cell_coord = DeviceAllocAndCopy(grid->cellCoord_vec, "cudaMalloc/cudaMemcpy cellCoord");
    size_t* d_cells_on_vertex = DeviceAllocAndCopy(grid->cellsOnVertex_vec, "cudaMalloc/cudaMemcpy cellsOnVertex");
    double* d_cell_center_attr = DeviceAllocAndCopy(cell_center_attr, "cudaMalloc/cudaMemcpy cellCenterAttr");
    double* d_cell_vertex_attr = nullptr;

    CheckCuda(cudaMalloc(reinterpret_cast<void**>(&d_cell_vertex_attr), cell_vertex_attr.size() * sizeof(double)), "cudaMalloc cellVertexAttr");
    ...
    CheckCuda(cudaGetLastError(), "KernelCalcCellCenterToVertex launch");
    CheckCuda(cudaDeviceSynchronize(), "KernelCalcCellCenterToVertex sync");

    CopyBackAndFree(cell_vertex_attr, d_cell_vertex_attr, "cudaMemcpy/cudaFree cellVertexAttr");
    FreeDev(d_cell_center_attr, "cudaFree cellCenterAttr");
    FreeDev(d_cells_on_vertex, "cudaFree cellsOnVertex");
    FreeDev(d_cell_coord, "cudaFree cellCoord");
    FreeDev(d_vertex_coord, "cudaFree vertexCoord");
}
    ...
    const std::vector<size_t>& grid_info)
{
    ...
    if (cell_zonal_velocity.size() < total || cell_meridional_velocity.size() < total) {
        Error("[CUDABackend::Kernel]::LaunchCalcCellCenterVelocityByZM input size mismatch");
    }

    cell_center_velocity.assign(total, vec3{0.0, 0.0, 0.0});

    vec3* d_cell_coord = DeviceAllocAndCopy(grid->cellCoord_vec, "cudaMalloc/cudaMemcpy cellCoord");
    double* d_cell_zonal = DeviceAllocAndCopy(cell_zonal_velocity, "cudaMalloc/cudaMemcpy cellZonalVelocity");
    double* d_cell_meridional = DeviceAllocAndCopy(cell_meridional_velocity, "cudaMalloc/cudaMemcpy cellMeridionalVelocity");
    vec3* d_cell_center_velocity = nullptr;

    CheckCuda(cudaMalloc(reinterpret_cast<void**>(&d_cell_center_velocity), total * sizeof(vec3)), "cudaMalloc cellCenterVelocity");
    ...
    CheckCuda(cudaGetLastError(), "KernelCalcCellCenterVelocityByZM launch");
    CheckCuda(cudaDeviceSynchronize(), "KernelCalcCellCenterVelocityByZM sync");

    CopyBackAndFree(cell_center_velocity, d_cell_center_velocity, "cudaMemcpy/cudaFree cellCenterVelocity");
    FreeDev(d_cell_meridional, "cudaFree cellMeridionalVelocity");
    FreeDev(d_cell_zonal, "cudaFree cellZonalVelocity");
    FreeDev(d_cell_coord, "cudaFree cellCoord");
}
    ...
    const std::vector<size_t>& grid_info)
{
    ...
    if (cell_vertex_zonal_velocity.size() < total || cell_vertex_meridional_velocity.size() < total) {
        Error("[CUDABackend::Kernel]::LaunchCalcCellVertexVelocityByZM input size mismatch");
    }
    if (grid->vertexCoord_vec.size() < total_vertex) {
        Error("[CUDABackend::Kernel]::LaunchCalcCellVertexVelocityByZM grid vertex size mismatch");
    }
    ...
    CheckCuda(cudaMalloc(reinterpret_cast<void**>(&d_vertex_coord), total_vertex * sizeof(vec3)), "cudaMalloc vertexCoord");
    CheckCuda(
        cudaMemcpy(
            d_vertex_coord,
        ...
        "cudaMemcpy vertexCoord");

    d_vertex_zonal = DeviceAllocAndCopy(cell_vertex_zonal_velocity, "cudaMalloc/cudaMemcpy vertexZonalVelocity");
    d_vertex_meridional = DeviceAllocAndCopy(cell_vertex_meridional_velocity, "cudaMalloc/cudaMemcpy vertexMeridionalVelocity");
    CheckCuda(cudaMalloc(reinterpret_cast<void**>(&d_vertex_velocity), total * sizeof(vec3)), "cudaMalloc vertexVelocity");
    ...
    CheckCuda(cudaGetLastError(), "KernelCalcCellVertexVelocityByZM launch");
    CheckCuda(cudaDeviceSynchronize(), "KernelCalcCellVertexVelocityByZM sync");

    CopyBackAndFree(cell_vertex_velocity, d_vertex_velocity, "cudaMemcpy/cudaFree vertexVelocity");
    FreeDev(d_vertex_meridional, "cudaFree vertexMeridionalVelocity");
    FreeDev(d_vertex_zonal, "cudaFree vertexZonalVelocity");
    FreeDev(d_vertex_coord, "cudaFree vertexCoord");
}
    ...
    const std::vector<size_t>& grid_info)
{
    ...
    if (grid->numberVertexOnCell_vec.size() < total_cells) {
        Error("[CUDABackend::Kernel]::LaunchCalcCellCenterVelocity numberVertexOnCell size mismatch");
    }
    if (cell_normal_velocity.empty()) {
        Error("[CUDABackend::Kernel]::LaunchCalcCellCenterVelocity input normal velocity is empty");
    }

    cell_center_velocity.assign(total, vec3{0.0, 0.0, 0.0});

    vec3* d_cell_coord = DeviceAllocAndCopy(grid->cellCoord_vec, "cudaMalloc/cudaMemcpy cellCoord");
    vec3* d_edge_coord = DeviceAllocAndCopy(grid->edgeCoord_vec, "cudaMalloc/cudaMemcpy edgeCoord");
    size_t* d_number_vertex_on_cell = DeviceAllocAndCopy(grid->numberVertexOnCell_vec, "cudaMalloc/cudaMemcpy numberVertexOnCell");
    size_t* d_edges_on_cell = DeviceAllocAndCopy(grid->edgesOnCell_vec, "cudaMalloc/cudaMemcpy edgesOnCell");
    size_t* d_cells_on_edge = DeviceAllocAndCopy(grid->cellsOnEdge_vec, "cudaMalloc/cudaMemcpy cellsOnEdge");
    double* d_cell_normal_velocity = DeviceAllocAndCopy(cell_normal_velocity, "cudaMalloc/cudaMemcpy cellNormalVelocity");
    vec3* d_cell_center_velocity = nullptr;
    CheckCuda(cudaMalloc(reinterpret_cast<void**>(&d_cell_center_velocity), total * sizeof(vec3)), "cudaMalloc cellCenterVelocity(normal)");
    ...
    CheckCuda(cudaGetLastError(), "KernelCalcCellCenterVelocity launch");
    CheckCuda(cudaDeviceSynchronize(), "KernelCalcCellCenterVelocity sync");

    CopyBackAndFree(cell_center_velocity, d_cell_center_velocity, "cudaMemcpy/cudaFree cellCenterVelocity(normal)");
    FreeDev(d_cell_normal_velocity, "cudaFree cellNormalVelocity");
    FreeDev(d_cells_on_edge, "cudaFree cellsOnEdge");
    FreeDev(d_edges_on_cell, "cudaFree edgesOnCell");
    FreeDev(d_number_vertex_on_cell, "cudaFree numberVertexOnCell");
    FreeDev(d_edge_coord, "cudaFree edgeCoord");
    FreeDev(d_cell_coord, "cudaFree cellCoord");
}
    ...
    const std::vector<size_t>& grid_info)
{
    ...
    if (cell_center_velocity.size() < total_cells * total_layer) {
        Error("[CUDABackend::Kernel]::LaunchCalcCellVertexVelocity input size mismatch");
    }
    ...
    vec3* d_cell_coord = DeviceAllocAndCopy(grid->cellCoord_vec, "cudaMalloc/cudaMemcpy cellCoord");
    size_t* d_cells_on_vertex = DeviceAllocAndCopy(grid->cellsOnVertex_vec, "cudaMalloc/cudaMemcpy cellsOnVertex");
    vec3* d_cell_center_velocity = DeviceAllocAndCopy(cell_center_velocity, "cudaMalloc/cudaMemcpy cellCenterVelocity(input)");
    vec3* d_cell_vertex_velocity = nullptr;
    CheckCuda(cudaMalloc(reinterpret_cast<void**>(&d_cell_vertex_velocity), cell_vertex_velocity.size() * sizeof(vec3)), "cudaMalloc cellVertexVelocity");
    ...
    CheckCuda(cudaGetLastError(), "KernelCalcCellVertexVelocity launch");
    CheckCuda(cudaDeviceSynchronize(), "KernelCalcCellVertexVelocity sync");

    CopyBackAndFree(cell_vertex_velocity, d_cell_vertex_velocity, "cudaMemcpy/cudaFree cellVertexVelocity");
    FreeDev(d_cell_center_velocity, "cudaFree cellCenterVelocity(input)");
    FreeDev(d_cells_on_vertex, "cudaFree cellsOnVertex");
    FreeDev(d_cell_coord, "cudaFree cellCoord");
    FreeDev(d_vertex_coord, "cudaFree vertexCoord");
}
    ...
    const std::vector<size_t>& grid_info)
{
    ...
    if (cell_center_vert_velocity.size() < total_cells * total_layer) {
        Error("[CUDABackend::Kernel]::LaunchCalcCellVertexVertVelocity input size mismatch");
    }
    ...
    vec3* d_cell_coord = DeviceAllocAndCopy(grid->cellCoord_vec, "cudaMalloc/cudaMemcpy cellCoord");
    size_t* d_cells_on_vertex = DeviceAllocAndCopy(grid->cellsOnVertex_vec, "cudaMalloc/cudaMemcpy cellsOnVertex");
    double* d_cell_center_vert_velocity = DeviceAllocAndCopy(cell_center_vert_velocity, "cudaMalloc/cudaMemcpy cellCenterVertVelocity");
    double* d_cell_vertex_vert_velocity = nullptr;
    CheckCuda(cudaMalloc(reinterpret_cast<void**>(&d_cell_vertex_vert_velocity), cell_vertex_vert_velocity.size() * sizeof(double)), "cudaMalloc cellVertexVertVelocity");
    ...
    CheckCuda(cudaGetLastError(), "KernelCalcCellVertexVertVelocity launch");
    CheckCuda(cudaDeviceSynchronize(), "KernelCalcCellVertexVertVelocity sync");

    CopyBackAndFree(cell_vertex_vert_velocity, d_cell_vertex_vert_velocity, "cudaMemcpy/cudaFree cellVertexVertVelocity");
    FreeDev(d_cell_center_vert_velocity, "cudaFree cellCenterVertVelocity");
    FreeDev(d_cells_on_vertex, "cudaFree cellsOnVertex");
    FreeDev(d_cell_coord, "cudaFree cellCoord");
    FreeDev(d_vertex_coord, "cudaFree vertexCoord");
}
