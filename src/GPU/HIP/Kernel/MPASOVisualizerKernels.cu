    ...
    double* img1)
{
    ...
    bool is_in_mesh = MOPS::HIPKernel::IsInMesh(
        cell,
        max_edge,
        current_position,
    ...
    MOPS::HIPKernel::GetCellVerticesIdx(
        cell,
    ...
    if (!MOPS::HIPKernel::GetCellVertexPos(vpos, current_cell_vertices_idx, MAX_VERTEX_NUM, current_cell_vertices_number, vertex_coord)) {
        MOPS::SetPixel(img0, width, height, y, x, vec3_nan);
        if (has_attr && img1 != nullptr) {
            MOPS::SetPixel(img1, width, height, y, x, vec3_nan);
        }
    ...
    vec3 v_top = MOPS::HIPKernel::CalcVelocity(
        current_cell_vertices_idx,
        w,
        MAX_VERTEX_NUM,
        current_cell_vertices_number,
    ...
    vec3 v_bot = MOPS::HIPKernel::CalcVelocity(
        current_cell_vertices_idx,
        w,
        MAX_VERTEX_NUM,
        current_cell_vertices_number,
    ...
    if (has_attr && img1 != nullptr && attr0 != nullptr && attr0_size > 0 && (attr0_size % actual_vertex_size) == 0) {
        ...
        double a0 = MOPS::HIPKernel::CalcAttribute(
            current_cell_vertices_idx,
            w,
            MAX_VERTEX_NUM,
            current_cell_vertices_number,
        ...
        if (attr1 != nullptr && attr1_size > 0 && (attr1_size % actual_vertex_size) == 0) {
            ...
            a1 = MOPS::HIPKernel::CalcAttribute(
                current_cell_vertices_idx,
                w,
                MAX_VERTEX_NUM,
                current_cell_vertices_number,
    ...
    double* img)
{
    ...
    if (!MOPS::HIPKernel::GetCellVertexPos(
    ...
    vec3 current_point_velocity = MOPS::HIPKernel::CalcVelocity(
        current_cell_vertices_idx,
        current_cell_vertex_weight,
        MAX_VERTEX_NUM,
        current_cell_vertices_number,
    ...
    ImageBuffer<double>* img)
{
    if (mpasoF == nullptr || config == nullptr || img == nullptr) {
        Error("[HIPBackend::Kernel]::VisualizeFixedLayer invalid input");
    }
    if (mpasoF->mGrid == nullptr || mpasoF->mSol_Front == nullptr) {
        Error("[HIPBackend::Kernel]::VisualizeFixedLayer field is not initialized");
    }
    ...
    MOPS::HIPKernel::SearchKDTree(cell_id_vec.data(), mpasoF->mGrid.get(), width, height, min_lat, max_lat, min_lon, max_lon);

    int* d_cell_id = DeviceAllocAndCopy(cell_id_vec, "hipMalloc/hipMemcpy cellID");
    vec3* d_vertex_coord = DeviceAllocAndCopy(mpasoF->mGrid->vertexCoord_vec, "hipMalloc/hipMemcpy vertexCoord");
    size_t* d_number_vertex_on_cell = DeviceAllocAndCopy(mpasoF->mGrid->numberVertexOnCell_vec, "hipMalloc/hipMemcpy numberVertexOnCell");
    size_t* d_vertices_on_cell = DeviceAllocAndCopy(mpasoF->mGrid->verticesOnCell_vec, "hipMalloc/hipMemcpy verticesOnCell");
    vec3* d_cell_vertex_velocity = DeviceAllocAndCopy(mpasoF->mSol_Front->cellVertexVelocity_vec, "hipMalloc/hipMemcpy cellVertexVelocity");

    double* d_img = nullptr;
    CheckHip(hipMalloc(reinterpret_cast<void**>(&d_img), img->mPixels.size() * sizeof(double)), "hipMalloc img");
    ...
    CheckHip(hipGetLastError(), "KernelVisualizeFixedLayer launch");
    CheckHip(hipDeviceSynchronize(), "KernelVisualizeFixedLayer sync");
    ...
    FreeDev(d_img, "hipFree img");
    FreeDev(d_cell_vertex_velocity, "hipFree cellVertexVelocity");
    FreeDev(d_vertices_on_cell, "hipFree verticesOnCell");
    FreeDev(d_number_vertex_on_cell, "hipFree numberVertexOnCell");
    FreeDev(d_vertex_coord, "hipFree vertexCoord");
    FreeDev(d_cell_id, "hipFree cellID");
}
    ...
    std::vector<ImageBuffer<double>>& img_vec)
{
    if (mpasoF == nullptr || config == nullptr || img_vec.empty()) {
        Error("[HIPBackend::Kernel]::VisualizeFixedDepth invalid input");
    }
    if (mpasoF->mGrid == nullptr || mpasoF->mSol_Front == nullptr) {
        Error("[HIPBackend::Kernel]::VisualizeFixedDepth field is not initialized");
    }
    ...
    size_t* d_vertices_on_cell = DeviceAllocAndCopy(mpasoF->mGrid->verticesOnCell_vec, "hipMalloc/hipMemcpy verticesOnCell");
    vec3* d_cell_vertex_velocity = DeviceAllocAndCopy(mpasoF->mSol_Front->cellVertexVelocity_vec, "hipMalloc/hipMemcpy cellVertexVelocity");
    double* d_cell_vertex_ztop = DeviceAllocAndCopy(mpasoF->mSol_Front->cellVertexZTop_vec, "hipMalloc/hipMemcpy cellVertexZTop");
    ...
    int attr1_size = 0;
    if (!attr_ptrs.empty()) {
        d_attr0 = DeviceAllocAndCopy(*attr_ptrs[0], "hipMalloc/hipMemcpy attr0");
        attr0_size = static_cast<int>(attr_ptrs[0]->size());
    }
    if (attr_ptrs.size() > 1) {
        d_attr1 = DeviceAllocAndCopy(*attr_ptrs[1], "hipMalloc/hipMemcpy attr1");
        attr1_size = static_cast<int>(attr_ptrs[1]->size());
    }

    double* d_img0 = nullptr;
    CheckHip(hipMalloc(reinterpret_cast<void**>(&d_img0), img_vec[0].mPixels.size() * sizeof(double)), "hipMalloc img0");

    double* d_img1 = nullptr;
    if (has_attr) {
        CheckHip(hipMalloc(reinterpret_cast<void**>(&d_img1), img_vec[1].mPixels.size() * sizeof(double)), "hipMalloc img1");
    }
    ...
    CheckHip(hipGetLastError(), "KernelVisualizeFixedDepth launch");
    CheckHip(hipDeviceSynchronize(), "KernelVisualizeFixedDepth sync");
    ...
    FreeDev(d_img1, "hipFree img1");
    FreeDev(d_img0, "hipFree img0");
    FreeDev(d_attr1, "hipFree attr1");
    FreeDev(d_attr0, "hipFree attr0");
    FreeDev(d_cell_vertex_ztop, "hipFree cellVertexZTop");
    FreeDev(d_cell_vertex_velocity, "hipFree cellVertexVelocity");
    FreeDev(d_vertices_on_cell, "hipFree verticesOnCell");
    FreeDev(d_number_vertex_on_cell, "hipFree numberVertexOnCell");
    FreeDev(d_vertex_coord, "hipFree vertexCoord");
...
{
    if (mpasoF == nullptr || config == nullptr || img == nullptr) {
        Error("[HIPBackend::Kernel]::VisualizeFixedLatitude invalid input");
    }
    if (mpasoF->mGrid == nullptr || mpasoF->mSol_Front == nullptr) {
        Error("[HIPBackend::Kernel]::VisualizeFixedLatitude field is not initialized");
    }
    ...
    if (actual_vertex_size <= 0 || ztop_size <= 0 || vel_size <= 0) {
        Error("[HIPBackend::Kernel]::VisualizeFixedLatitude invalid mesh/solution array sizes");
    }

    int n_vert = ztop_size / actual_vertex_size;
    if (n_vert <= 0 || n_vert > MAX_VERTLEVELS || (vel_size / actual_vertex_size) < n_vert) {
        Error("[HIPBackend::Kernel]::VisualizeFixedLatitude invalid vertical levels");
    }
    ...
    for (int ih = 0; ih < height; ++ih) {
        ...
        for (int jw = 0; jw < width; ++jw) {
            ...
            vec3 current_point_vel_up = MOPS::HIPKernel::CalcVelocity(
                current_cell_vertices_idx,
                current_cell_vertex_weight,
                MAX_VERTEX_NUM,
                current_cell_vertices_number,
            ...
            vec3 current_point_vel_dn = MOPS::HIPKernel::CalcVelocity(
                current_cell_vertices_idx,
                current_cell_vertex_weight,
                MAX_VERTEX_NUM,
                current_cell_vertices_number,
...
MOPS_DEVICE inline vec3 AdvectOnSphereCUDA(const vec3& pos, const vec3& vel, double dt_local)
{
    ...
    vec3 axis = MOPS::HIPKernel::CalcRotationAxis(pos, vel);
    const double theta = (speed_local * dt_local) / rr;
    return MOPS::HIPKernel::CalcPositionAfterRotation(pos, axis, theta);
}
    ...
    const double* cell_vertex_vert_velocity)
{
    ...
    double w_dn = MOPS::HIPKernel::CalcAttribute(
        current_cell_vertices_idx,
        current_cell_vertex_weight,
        MAX_VERTEX_NUM,
        current_cell_vertices_number,
    ...
    double w_up = MOPS::HIPKernel::CalcAttribute(
        current_cell_vertices_idx,
        current_cell_vertex_weight,
        MAX_VERTEX_NUM,
        current_cell_vertices_number,
    ...
    const double* cell_vertex_vert_velocity)
{
    ...
    for (int times_i = 0; times_i < times; ++times_i) {
        ...
        if (first_loop) {
            ...
            MOPS::HIPKernel::GetCellNeighborsIdx(
                cell_id,
                current_cell_vertices_number,
                cell_neig_vec,
        ...
        if (use_euler) {
            vec3 rotation_axis = MOPS::HIPKernel::CalcRotationAxis(current_position, current_horizontal_velocity);
            double speed = MOPS_LENGTH(current_horizontal_velocity);
            double theta_rad = (speed * delta_t) / r;
            new_position = MOPS::HIPKernel::CalcPositionAfterRotation(current_position, rotation_axis, theta_rad);
        } else {
    ...
    std::vector<int>& default_cell_id)
{
    if (mpasoF == nullptr || config == nullptr) {
        Error("[HIPBackend::Kernel]::StreamLine invalid input");
    }
    if (mpasoF->mGrid == nullptr || mpasoF->mSol_Front == nullptr) {
        Error("[HIPBackend::Kernel]::StreamLine field is not initialized");
    }
    ...
    if (config->deltaT == 0 || config->recordT == 0 || config->simulationDuration == 0) {
        Error("[HIPBackend::Kernel]::StreamLine invalid trajectory settings");
    }
    ...
    if (grid_info_vec.size() < 6) {
        Error("[HIPBackend::Kernel]::StreamLine grid_info is incomplete");
    }
    ...
    vec3* d_sample_points = DeviceAllocAndCopy(stable_points, "hipMalloc/hipMemcpy sample_points");
    vec3* d_write_points = DeviceAllocAndCopy(update_points, "hipMalloc/hipMemcpy write_points");
    vec3* d_write_vels = DeviceAllocAndCopy(update_vels, "hipMalloc/hipMemcpy write_vels");
    float* d_particle_depths = DeviceAllocAndCopy(effective_depths, "hipMalloc/hipMemcpy particle_depths");
    int* d_default_cell_id = DeviceAllocAndCopy(default_cell_id, "hipMalloc/hipMemcpy default_cell_id");

    vec3* d_vertex_coord = DeviceAllocAndCopy(mpasoF->mGrid->vertexCoord_vec, "hipMalloc/hipMemcpy vertexCoord");
    vec3* d_cell_coord = DeviceAllocAndCopy(mpasoF->mGrid->cellCoord_vec, "hipMalloc/hipMemcpy cellCoord");
    size_t* d_number_vertex_on_cell = DeviceAllocAndCopy(mpasoF->mGrid->numberVertexOnCell_vec, "hipMalloc/hipMemcpy numberVertexOnCell");
    size_t* d_vertices_on_cell = DeviceAllocAndCopy(mpasoF->mGrid->verticesOnCell_vec, "hipMalloc/hipMemcpy verticesOnCell");
    size_t* d_cells_on_cell = DeviceAllocAndCopy(mpasoF->mGrid->cellsOnCell_vec, "hipMalloc/hipMemcpy cellsOnCell");

    vec3* d_cell_vertex_velocity = DeviceAllocAndCopy(mpasoF->mSol_Front->cellVertexVelocity_vec, "hipMalloc/hipMemcpy cellVertexVelocity");
    double* d_cell_vertex_ztop = DeviceAllocAndCopy(mpasoF->mSol_Front->cellVertexZTop_vec, "hipMalloc/hipMemcpy cellVertexZTop");
    double* d_cell_vertex_vert_velocity = DeviceAllocAndCopy(mpasoF->mSol_Front->cellVertexVertVelocity_vec, "hipMalloc/hipMemcpy cellVertexVertVelocity");
    ...
    CheckHip(hipGetLastError(), "KernelStreamLine launch");
    CheckHip(hipDeviceSynchronize(), "KernelStreamLine sync");
    ...
    CheckHip(
        hipMemcpy(
            ...
            hipMemcpyDeviceToHost),
        "hipMemcpy streamline velocities");

    FreeDev(d_cell_vertex_vert_velocity, "hipFree cellVertexVertVelocity");
    FreeDev(d_cell_vertex_ztop, "hipFree cellVertexZTop");
    FreeDev(d_cell_vertex_velocity, "hipFree cellVertexVelocity");
    FreeDev(d_cells_on_cell, "hipFree cellsOnCell");
    FreeDev(d_vertices_on_cell, "hipFree verticesOnCell");
    FreeDev(d_number_vertex_on_cell, "hipFree numberVertexOnCell");
    FreeDev(d_cell_coord, "hipFree cellCoord");
    FreeDev(d_vertex_coord, "hipFree vertexCoord");
    FreeDev(d_default_cell_id, "hipFree defaultCellID");
    FreeDev(d_particle_depths, "hipFree particleDepths");
    FreeDev(d_write_vels, "hipFree writeVels");
    FreeDev(d_write_points, "hipFree writePoints");
    FreeDev(d_sample_points, "hipFree samplePoints");
    ...
    const double* attr1_back)
{
    ...
    vec3 current_point_vel_dn_front = MOPS::HIPKernel::CalcVelocity(
        current_cell_vertices_idx,
        current_cell_vertex_weight,
        MAX_VERTEX_NUM,
        current_cell_vertices_number,
    ...
    vec3 current_point_vel_up_front = MOPS::HIPKernel::CalcVelocity(
        current_cell_vertices_idx,
        current_cell_vertex_weight,
        MAX_VERTEX_NUM,
        current_cell_vertices_number,
    ...
    vec3 current_point_vel_dn_back = MOPS::HIPKernel::CalcVelocity(
        current_cell_vertices_idx,
        current_cell_vertex_weight,
        MAX_VERTEX_NUM,
        current_cell_vertices_number,
    ...
    vec3 current_point_vel_up_back = MOPS::HIPKernel::CalcVelocity(
        current_cell_vertices_idx,
        current_cell_vertex_weight,
        MAX_VERTEX_NUM,
        current_cell_vertices_number,
    ...
    double w_dn_front = MOPS::HIPKernel::CalcAttribute(
        current_cell_vertices_idx,
        current_cell_vertex_weight,
        MAX_VERTEX_NUM,
        current_cell_vertices_number,
    ...
    double w_up_front = MOPS::HIPKernel::CalcAttribute(
        current_cell_vertices_idx,
        current_cell_vertex_weight,
        MAX_VERTEX_NUM,
        current_cell_vertices_number,
    ...
    double w_dn_back = MOPS::HIPKernel::CalcAttribute(
        current_cell_vertices_idx,
        current_cell_vertex_weight,
        MAX_VERTEX_NUM,
        current_cell_vertices_number,
    ...
    double w_up_back = MOPS::HIPKernel::CalcAttribute(
        current_cell_vertices_idx,
        current_cell_vertex_weight,
        MAX_VERTEX_NUM,
        current_cell_vertices_number,
    ...
    if (has_double_attributes) {
        if (attr_count >= 1 && attr0_front != nullptr && attr0_back != nullptr) {
            double attr_dn_front = MOPS::HIPKernel::CalcAttribute(
                current_cell_vertices_idx,
                current_cell_vertex_weight,
                MAX_VERTEX_NUM,
                current_cell_vertices_number,
                ...
                local_layer_front,
                attr0_front);
            double attr_up_front = MOPS::HIPKernel::CalcAttribute(
                current_cell_vertices_idx,
                current_cell_vertex_weight,
                MAX_VERTEX_NUM,
                current_cell_vertices_number,
            ...
            double attr_dn_back = MOPS::HIPKernel::CalcAttribute(
                current_cell_vertices_idx,
                current_cell_vertex_weight,
                MAX_VERTEX_NUM,
                current_cell_vertices_number,
                ...
                local_layer_back,
                attr0_back);
            double attr_up_back = MOPS::HIPKernel::CalcAttribute(
                current_cell_vertices_idx,
                current_cell_vertex_weight,
                MAX_VERTEX_NUM,
                current_cell_vertices_number,
...
{
    if (mpasoF == nullptr || config == nullptr) {
        Error("[HIPBackend::Kernel]::PathLine invalid input");
    }
    if (mpasoF->mGrid == nullptr || mpasoF->mSol_Front == nullptr || mpasoF->mSol_Back == nullptr) {
        Error("[HIPBackend::Kernel]::PathLine field is not initialized");
    }
    ...
    if (config->deltaT == 0 || config->recordT == 0 || config->simulationDuration == 0) {
        Error("[HIPBackend::Kernel]::PathLine invalid trajectory settings");
    }
    ...
    std::vector<size_t> grid_info_vec = MOPS::Common::BuildGridInfo(mpasoF->mGrid.get());
    if (grid_info_vec.size() < 6) {
        Error("[HIPBackend::Kernel]::PathLine grid_info is incomplete");
    }
    ...
    vec3* d_sample_points = DeviceAllocAndCopy(stable_points, "hipMalloc/hipMemcpy path sample_points");
    vec3* d_write_points = DeviceAllocAndCopy(update_points, "hipMalloc/hipMemcpy path write_points");
    vec3* d_write_vels = DeviceAllocAndCopy(update_vels, "hipMalloc/hipMemcpy path write_vels");
    vec3* d_write_attrs = DeviceAllocAndCopy(update_attrs, "hipMalloc/hipMemcpy path write_attrs");
    float* d_particle_depths = DeviceAllocAndCopy(effective_depths, "hipMalloc/hipMemcpy path particle_depths");
    int* d_default_cell_id = DeviceAllocAndCopy(default_cell_id, "hipMalloc/hipMemcpy path default_cell_id");

    vec3* d_vertex_coord = DeviceAllocAndCopy(mpasoF->mGrid->vertexCoord_vec, "hipMalloc/hipMemcpy path vertexCoord");
    vec3* d_cell_coord = DeviceAllocAndCopy(mpasoF->mGrid->cellCoord_vec, "hipMalloc/hipMemcpy path cellCoord");
    size_t* d_number_vertex_on_cell = DeviceAllocAndCopy(mpasoF->mGrid->numberVertexOnCell_vec, "hipMalloc/hipMemcpy path numberVertexOnCell");
    size_t* d_vertices_on_cell = DeviceAllocAndCopy(mpasoF->mGrid->verticesOnCell_vec, "hipMalloc/hipMemcpy path verticesOnCell");
    size_t* d_cells_on_cell = DeviceAllocAndCopy(mpasoF->mGrid->cellsOnCell_vec, "hipMalloc/hipMemcpy path cellsOnCell");

    vec3* d_cell_vertex_velocity_front = DeviceAllocAndCopy(mpasoF->mSol_Front->cellVertexVelocity_vec, "hipMalloc/hipMemcpy path cellVertexVelocityFront");
    vec3* d_cell_vertex_velocity_back = DeviceAllocAndCopy(mpasoF->mSol_Back->cellVertexVelocity_vec, "hipMalloc/hipMemcpy path cellVertexVelocityBack");
    double* d_cell_vertex_ztop_front = DeviceAllocAndCopy(mpasoF->mSol_Front->cellVertexZTop_vec, "hipMalloc/hipMemcpy path cellVertexZTopFront");
    double* d_cell_vertex_ztop_back = DeviceAllocAndCopy(mpasoF->mSol_Back->cellVertexZTop_vec, "hipMalloc/hipMemcpy path cellVertexZTopBack");
    double* d_cell_vertex_vert_velocity_front = DeviceAllocAndCopy(mpasoF->mSol_Front->cellVertexVertVelocity_vec, "hipMalloc/hipMemcpy path cellVertexVertVelocityFront");
    double* d_cell_vertex_vert_velocity_back = DeviceAllocAndCopy(mpasoF->mSol_Back->cellVertexVertVelocity_vec, "hipMalloc/hipMemcpy path cellVertexVertVelocityBack");
    ...
    if (attr_count >= 1) {
        d_attr0_front = DeviceAllocAndCopy(*front_attr_ptrs[0], "hipMalloc/hipMemcpy path attr0Front");
        d_attr0_back = DeviceAllocAndCopy(*back_attr_ptrs[0], "hipMalloc/hipMemcpy path attr0Back");
    }
    if (attr_count >= 2) {
        d_attr1_front = DeviceAllocAndCopy(*front_attr_ptrs[1], "hipMalloc/hipMemcpy path attr1Front");
        d_attr1_back = DeviceAllocAndCopy(*back_attr_ptrs[1], "hipMalloc/hipMemcpy path attr1Back");
    }
    ...
    CheckHip(hipGetLastError(), "KernelPathLine launch");
    CheckHip(hipDeviceSynchronize(), "KernelPathLine sync");
    ...
    CheckHip(
        hipMemcpy(
            ...
            hipMemcpyDeviceToHost),
        "hipMemcpy path attrs");

    FreeDev(d_attr1_back, "hipFree path attr1Back");
    FreeDev(d_attr1_front, "hipFree path attr1Front");
    FreeDev(d_attr0_back, "hipFree path attr0Back");
    FreeDev(d_attr0_front, "hipFree path attr0Front");
    FreeDev(d_cell_vertex_vert_velocity_back, "hipFree path cellVertexVertVelocityBack");
    FreeDev(d_cell_vertex_vert_velocity_front, "hipFree path cellVertexVertVelocityFront");
    FreeDev(d_cell_vertex_ztop_back, "hipFree path cellVertexZTopBack");
    FreeDev(d_cell_vertex_ztop_front, "hipFree path cellVertexZTopFront");
    FreeDev(d_cell_vertex_velocity_back, "hipFree path cellVertexVelocityBack");
    FreeDev(d_cell_vertex_velocity_front, "hipFree path cellVertexVelocityFront");
    FreeDev(d_cells_on_cell, "hipFree path cellsOnCell");
    FreeDev(d_vertices_on_cell, "hipFree path verticesOnCell");
    FreeDev(d_number_vertex_on_cell, "hipFree path numberVertexOnCell");
    FreeDev(d_cell_coord, "hipFree path cellCoord");
    FreeDev(d_vertex_coord, "hipFree path vertexCoord");
    FreeDev(d_default_cell_id, "hipFree path defaultCellID");
    FreeDev(d_particle_depths, "hipFree path particleDepths");
    FreeDev(d_write_attrs, "hipFree path writeAttrs");
    FreeDev(d_write_vels, "hipFree path writeVels");
    FreeDev(d_write_points, "hipFree path writePoints");
    FreeDev(d_sample_points, "hipFree path samplePoints");
