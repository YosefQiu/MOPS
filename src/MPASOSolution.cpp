...
void MPASOSolution::calcCellCenterVelocity(MPASOGrid* grid, sycl::queue& q)
{
    ...
    q.submit([&](sycl::handler& cgh) {
        ...
        cgh.parallel_for(sycl::range<2>(mCellsSize, mTotalZTopLayer), [=](sycl::id<2> idx) {
            ...
            int pointCount = VERTEX_NUM; 
            double edge_center[VERTEX_NUM][3];
            double unit_vector[VERTEX_NUM][3];
            double noraml_vel[VERTEX_NUM][1];
            double coeffs[VERTEX_NUM][3] = {{0.0}};
            // 3. 遍历 -》 计算每条边的矢量速度
            for (auto k = 0; k < VERTEX_NUM; ++k)
            {
...
void MPASOSolution::calcCellCenterVelocityByZM(MPASOGrid *grid, sycl::queue &q)
{
    if (!cellCenterVelocity_vec.empty()) cellCenterVelocity_vec.clear();
    cellCenterVelocity_vec.resize(grid->mCellsSize * mTotalZTopLayer);
