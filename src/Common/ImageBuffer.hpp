...
namespace MOPS
{
    ...
    MOPS_HOST_DEVICE inline void SetPixel(Accessor img_acc, const int w, const int h, const int i, const int j, const vec3& val)
    {
    ...
    template<typename Accessor>
    MOPS_HOST_DEVICE inline void GetPixel(Accessor img_acc, const int w, const int h, const int i, const int j, vec3& val)
    {
        if (i < 0 || i >= h || j < 0 || j >= w) return;
        auto index = (i * w + j) * 4;
